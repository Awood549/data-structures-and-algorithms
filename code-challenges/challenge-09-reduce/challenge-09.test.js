...
const reversedString = (arr) => {
  let str = arr.split('');
  let reversed = [];
  str.reduce(function(accumulator,value,idx){
    accumulator = value;
...
const countNumberOfChildren = (arr) => {
  let filtered = arr.filter( v => v.children);
  let newArr = filtered.map(v => {
    return v.children.length;
  });
  return newArr.reduce(function (accumulator,value,idx) {
...
const countPrimeNumbers = (arr) => {
  return arr.reduce( function(accumulator,value,idx) {
    accumulator = accumulator+isPrime(value);
    return accumulator;
  }, 0);
};
