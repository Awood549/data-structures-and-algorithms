...
const toTitleCase = (arr) => {
  ...
  arr.forEach(function(element){
    myArr.push(element.replace(element[0],element.charAt(0).toUpperCase()));
  });
  return myArr;};
...
let biggerThanLuke = (arr) => {
  return arr.filter(element=>parseInt(element.height)>172).map(element=>element.name).join().replace(',',' - ');};
...
const sortBy = (property, arr) => {
  return arr.sort(function(ele1, ele2) {
    if (ele1[property] < ele2[property]) {return -1;
    }
    if (ele1[property] > ele2[property]) {return 1;
    }
  });};
